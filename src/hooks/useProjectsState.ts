...
const projectsHaveChanges = (
	...
	return nextProjects.some((nextProject, index) => {
		...
		return (
			...
			serialize(nextProject.sessionMeta) !==
				serialize(prevProject.sessionMeta) ||
...
const mergeTaskMasterCache = (
	...
	return nextProjects.map((project) => {
		const cachedTaskMasterInfo = previousTaskMasterByProject.get(
			project.projectId,
		);
...
const countLoadedProjectSessions = (project: Project): number =>
	getProjectSessions(project).length;

const mergeSessionProviderLists = (
	baseSessions: ProjectSession[],
	additionalSessions: ProjectSession[],
): ProjectSession[] => {
	const merged = [...baseSessions];
	const seenSessionIds = new Set(
		baseSessions.map((session) => String(session.id)),
	);
...
const mergeExpandedSessionPages = (
	previousProjects: Project[],
	...
	const previousByProjectId = new Map(
		previousProjects.map((project) => [project.projectId, project]),
	);
	...
	return incomingProjects.map((incomingProject) => {
		...
		const mergedProject: Project = {
			...
			sessions: mergeSessionProviderLists(
				...
				previousProject.sessions ?? [],
			),
		};

		const totalSessions = Number(
			incomingProject.sessionMeta?.total ?? previousLoadedCount,
		);
...
const mergeProjectSessionPage = (
	...
	const totalSessions = Number(
		sessionsPage.sessionMeta?.total ?? existingProject.sessionMeta?.total ?? 0,
	);
	mergedProject.sessionMeta = {
 ...
 */
const upsertSessionIntoProject = (
	project: Project,
	event: SessionUpsertedEvent,
): Project => {
	...
	const existingIndex = sessions.findIndex((session) =>
		aliasIds.has(String(session.id)),
	);
...
const removeSessionFromProject = (
	project: Project,
	...
	const totalSessions = Math.max(
		0,
		Number(project.sessionMeta?.total ?? 0) - 1,
	);
...
export function useProjectsState({
	...
	const loadingProgressTimeoutRef = useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	/**
		...
		async ({ showLoadingState = true }: FetchProjectsOptions = {}) => {
			try {
				...
				setProjects((prevProjects) => {
					const projectsWithTaskMaster = mergeTaskMasterCache(
						projectData,
						prevProjects,
					);
					const mergedProjects = mergeExpandedSessionPages(
						prevProjects,
	...
	const registerOptimisticSession = useCallback(
		({
			...
			setProjects((previousProjects) => {
				...
				if (!existingProject) {
					return [
						upsertSessionIntoProject(projectFromRegistration(project), upsert),
						...previousProjects,
					];
				}

				const updatedProject = upsertSessionIntoProject(
					existingProject,
					upsert,
				);
				...
				return previousProjects.map((candidate) =>
					candidate.projectId === existingProject.projectId
						? updatedProject
						: candidate,
				);
			});
			...
			setSelectedProject((previousProject) => {
				...
				return updatedProject === previousProject
					? previousProject
					: updatedProject;
			});
	...
	const hydrateProjectTaskMaster = useCallback(async (projectId: string) => {
		...
		try {
			...
			console.error(
				`Error fetching TaskMaster info for project ${projectId}:`,
				error,
			);
		}
	}, []);
		...
		const handleEvent = (event: ServerEvent) => {
				...
				const existingProject = previousProjects.find((project) =>
					targetProjectId
						...
						: getProjectSessions(project).some(
								(session) => session.id === upsert.sessionId,
							),
				);
			...
			setSelectedProject((previousProject) => {
				...
				const matches = upsert.project
					...
					: getProjectSessions(previousProject).some(
							(session) => session.id === upsert.sessionId,
						);
				if (!matches) {
					return previousProject;
			...
			const aliasedSelectedSessionId =
				typeof upsert.providerSessionId === "string" &&
				upsert.providerSessionId !== upsert.sessionId
			...
			const normalizedSelectedSession: ProjectSession = {
				...
				__projectId:
					upsert.project?.projectId ?? currentSelectedSession?.__projectId,
			};
		...
		(sessionIdToDelete: string) => {
			...
			setProjects((prevProjects) =>
				prevProjects.map((project) =>
					removeSessionFromProject(project, sessionIdToDelete),
				),
			);
		},
	...
	const handleSidebarRefresh = useCallback(async () => {
		try {
			...
			setProjects((prevProjects) =>
				projectsHaveChanges(prevProjects, mergedProjects)
					? mergedProjects
					: prevProjects,
			);
			...
			if (refreshedSession) {
				...
				if (
					serialize(normalizedRefreshedSession) !== serialize(selectedSession)
				) {
		...
		async (projectId: string) => {
			...
			setProjects((previousProjects) =>
				previousProjects.map((candidate) => {
					...
					const mergedProject = mergeProjectSessionPage(
						candidate,
						sessionsPage,
					);
