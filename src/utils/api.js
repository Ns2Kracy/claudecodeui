...
export const storeAuthToken = (token) => {
	...
	if (typeof window !== "undefined") {
		window.dispatchEvent(
			new CustomEvent(AUTH_TOKEN_REFRESHED_EVENT, { detail: token }),
		);
	}
	return true;
};
...
export const api = {
	...
	unifiedSessionMessages: (
		...
		return authenticatedFetch(
			`/api/providers/sessions/${encodeURIComponent(sessionId)}/messages${queryString ? `?${queryString}` : ""}`,
		);
	},
	...
	deleteSession: (sessionId, hardDelete = false) => {
		...
		return authenticatedFetch(
			`/api/providers/sessions/${sessionId}${qs ? `?${qs}` : ""}`,
			{
				method: "DELETE",
			},
		);
	...
	runningSessions: () => authenticatedFetch("/api/providers/sessions/running"),
	providerSessionId: (sessionId) =>
		authenticatedFetch(
			`/api/providers/sessions/${encodeURIComponent(sessionId)}/provider-id`,
		),
	...
	readFile: (projectId, filePath) =>
		authenticatedFetch(
			`/api/file-tree/projects/${projectId}/file?filePath=${encodeURIComponent(filePath)}`,
		),
	readFileBlob: (projectId, filePath) =>
		authenticatedFetch(
			`/api/file-tree/projects/${projectId}/files/content?path=${encodeURIComponent(filePath)}`,
		),
	...
	getMentionableFiles: (projectId, options = {}) =>
		authenticatedFetch(
			`/api/file-tree/projects/${projectId}/files?respectGitignore=true`,
			options,
		),
