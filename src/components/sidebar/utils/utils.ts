...
export const getSessionDate = (session: SessionWithProvider): Date => {
	return new Date(
		getUpdatedTimestamp(session) || getCreatedTimestamp(session) || 0,
	);
};
...
export const sortProjects = (
	...
	byName.sort((projectA, projectB) => {
		...
		return (projectA.displayName || projectA.projectId).localeCompare(
...
export const getTaskIndicatorStatus = (
	...
	const mcpConfigured = Boolean(
		mcpServerStatus?.hasMCPServer && mcpServerStatus?.isConfigured,
	);
...
export const normalizeProjectForSettings = (
	project: Project,
): SettingsProject => {
	const fallbackPath =
