...
import { routingApi, RoutingApiError } from '../../settings/view/tabs/nine-router-settings/routingApi.js';
    ...
    try {
      const session = await startCodexRoutingOAuth({
        ...
        openPopup: (url: string) =>
          window.open(url, 'cloudcli-9router-oauth', 'popup,width=720,height=760'),
      });
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 max-md:items-stretch max-md:justify-stretch">
      ...
      >
        <div className="flex items-center justify-between border-b border-border p-4">
          <h3 id="codex-login-title" className="text-lg font-semibold text-foreground">
            Codex Login
          </h3>
          <button
            type="button"
          ...
          {isAuthenticated ? (
            ...
            <>
              <div className="space-y-2">
                <p className="text-sm font-medium text-foreground">Connect Codex through 9Router</p>
                <p className="text-sm text-muted-foreground">
                  Authorization and credentials are managed by the routing layer. CloudCLI does not store your OpenAI OAuth token.
                </p>
              </div>
              ...
              >
                ...
                {busy ? 'Connecting…' : error ? 'Try OAuth again' : 'Continue with OAuth'}
              </button>
            </>
          )}
        </div>
