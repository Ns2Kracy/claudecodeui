...
const PROVIDER_NAMES: Record<SkillsProvider, string> = { codex: "Codex" };
const CODEX_SKILL_PATH = "~/.agents/skills/<skill-name>/SKILL.md";
...
const buildQueuedSkillFolders = (selectedFiles: File[]): QueuedSkillFile[] => {
	...
	const skillRoots = files
		.filter(
			({ relativePath }) =>
				getBaseName(relativePath).toLowerCase() === "skill.md",
		)
...
export default function ProviderSkills({
	...
	const providerPath = CODEX_SKILL_PATH;
