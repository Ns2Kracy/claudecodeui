...
type UseShellConnectionOptions = {
	...
	onProcessCompleteRef: MutableRefObject<
		((exitCode: number) => void) | null | undefined
	>;
...
export function useShellConnection({
	...
	const connectWebSocket = useCallback(
		(isConnectionLocked = false) => {
			if (
				(connectingRef.current && !isConnectionLocked) ||
				isConnecting ||
				isConnected
			) {
				return;
					...
					window.setTimeout(() => {
						...
						sendSocketMessage(socket, {
							...
							hasSession: isPlainShellRef.current
								? false
								: Boolean(selectedSessionRef.current),
							provider: isPlainShellRef.current ? "plain-shell" : "codex",
	...
	const disconnectFromShell = useCallback(
		(options?: { suppressAutoConnect?: boolean }) => {
