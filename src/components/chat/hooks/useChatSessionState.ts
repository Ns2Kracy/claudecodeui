	...
	return {
		...
		images:
			Array.isArray(msg.images) && msg.images.length > 0
				...
				: undefined,
		files:
			Array.isArray(msg.files) && msg.files.length > 0 ? msg.files : undefined,
	} as NormalizedMessage;
}
...
export function useChatSessionState({
	...
	const [tokenBudget, setTokenBudget] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const loadAllFinishedTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	const loadAllOverlayTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	const createDiff = useMemo<DiffCalculator>(
		() => createCachedDiffCalculator(),
		[],
	);
	...
	const sessionActivity =
		(activeSessionId && processingSessions?.get(activeSessionId)) || null;
	...
	const rewindMessages = useCallback(
		(count: number) => setViewHiddenCount(count),
		[],
	);
	...
	useEffect(() => {
		...
		const sessionChanged =
			currentSessionId !== null && currentSessionId !== selectedSessionId;
		if (sessionChanged) {
			resetStreamingState();
		}
		...
		sessionStore
			.fetchFromServer(selectedSessionId, {
				limit: MESSAGES_PER_PAGE,
				offset: 0,
			})
			.then((slot) => {
	...
	useEffect(() => {
		...
		if (typeof targetSnippet === "string" && targetSnippet) {
			...
			setSearchTarget({
				...
				timestamp:
					typeof targetTimestamp === "string" ? targetTimestamp : undefined,
			});
		}
	}, [selectedSession]);
	...
	useEffect(() => {
		if (!searchTarget || chatMessages.length === 0 || isLoadingSessionMessages)
			return;
