...
const isImageAttachment = (attachment: ChatAttachment) => {
	...
	return /\.(gif|jpe?g|png|svg|webp)$/i.test(
...
const uploadAttachmentFiles = async (files: File[]): Promise<unknown[]> => {
	...
	if (
		...
		result.attachments.length !== files.length
	) {
...
export function useChatComposerState({
	...
	const [input, setInput] = useState(() => {
		if (typeof window !== "undefined" && selectedProject) {
			...
			return (
				safeLocalStorage.getItem(`draft_input_${selectedProject.projectId}`) ||
				""
			);
		}
		return "";
	...
	const [commandModalPayload, setCommandModalPayload] =
		useState<CommandModalPayload | null>(null);
	...
	const processingSessionsRef = useRef<SessionActivityMap | undefined>(
		processingSessions,
	);
	...
	const resizeTextarea = useCallback((target: HTMLTextAreaElement) => {
		...
		if (!lineHeight) {
			...
			textareaLineHeightRef.current = Number.isFinite(lineHeight)
				? lineHeight
				: 24;
		}

		const expanded = nextHeight > (textareaLineHeightRef.current || 24) * 2;
		setIsTextareaExpanded((previous) =>
	...
	const handleAttachmentFiles = useCallback((files: File[]) => {
		...
		if (validFiles.length > 0) {
			setAttachedFiles((previous) =>
				[...previous, ...validFiles].slice(0, MAX_ATTACHMENT_COUNT),
			);
		}
	}, []);
	...
	const buildSendOptions = useCallback(
		(currentInput: string): QueuedSendOptions => {
			const getToolsSettings = () => {
				try {
					const savedSettings = safeLocalStorage.getItem("codex-settings");
					if (savedSettings) {
			...
			return {
				...
				permissionMode: resolvePermissionModeForProvider(
					provider,
					permissionMode,
				),
				toolsSettings,
		...
		async (
			...
			if (isLoading) {
				...
				if (queuedSessionKey && sessionKeyRef.current !== queuedSessionKey) {
					if (
						...
						onSessionProcessing?.(queuedSessionKey, {
							statusText: null,
							canInterrupt: true,
						});
					}
			...
			if (commandInput.startsWith("/") || isHelpAlias) {
				...
				const commandName = isHelpAlias
					? "/help"
					: firstSpace > 0
						? commandInput.slice(0, firstSpace)
						: commandInput;
			...
			const userMessage: ChatMessage = {
				...
				files: attachmentRecords.filter(
	...
	useEffect(() => {
		...
		const savedInput =
			safeLocalStorage.getItem(`draft_input_${selectedProjectId}`) || "";
		setInput((previous) => {
