...
const MessageCopyControl = ({
	...
	const openDropdown = () => {
		...
		if (rect) {
			...
			const openUp =
				rect.bottom + ESTIMATED_MENU_HEIGHT + 8 > window.innerHeight;
			setMenuStyle({
				position: "fixed",
	...
	const copyFormatOptions: CopyFormatOption[] = useMemo(
		() => [
			{
				format: "markdown",
				label: t("copyMessage.copyAsMarkdown", {
			...
			{canSelectCopyFormat && (
				<>
					<button
						...
						aria-label={t("copyMessage.selectFormat", {
							defaultValue: "Select copy format",
						})}
						title={t("copyMessage.selectFormat", {
							defaultValue: "Select copy format",
						})}
					>
						<svg
