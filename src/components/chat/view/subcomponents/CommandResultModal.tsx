...
type CommandResultModalProps = {
	...
	providerModelCacheCatalog: Partial<
...
const FALLBACK_COMMANDS: CommandEntry[] = [
	...
	{
		name: "/status",
		description: "Inspect runtime, version, provider, and environment status.",
	},
...
function MetricCard({
	...
	return (
		...
		>
			<div
				className={`inline-flex rounded-xl border ${compact ? "mb-2 p-1.5" : "mb-3 p-2"} ${toneClass}`}
			>
			...
			<p
				className={`${compact ? "mt-0.5 text-[13px]" : "mt-1 text-sm"} break-all font-semibold text-foreground`}
			>
				{value}
			</p>
		</div>
...
function HelpContent({ data }: { data: HelpCommandData }) {
	...
	const filteredCommands = useMemo(() => {
		...
		return commands.filter((command) => {
			const haystack =
				`${command.name} ${command.description || ""} ${command.namespace || ""}`.toLowerCase();
			...
			<aside className="space-y-3">
				<div className="rounded-2xl border border-border/70 bg-muted/20 p-4">
					...
					<div className="space-y-2 text-sm text-muted-foreground">
						<p>
							<code className="text-foreground">/command arg1 arg2</code>
						</p>
						<p>
							<code className="text-foreground">$ARGUMENTS</code> passes all
							args.
						</p>
						<p>
							<code className="text-foreground">$1</code>,{" "}
							<code className="text-foreground">$2</code> pass positional args.
						</p>
						<p>
							<code className="text-foreground">@file</code> includes file
							contents.
						</p>
					</div>
				</div>
				...
				<div className="rounded-2xl border border-primary/25 bg-primary/10 p-4">
					...
					<p className="text-sm leading-5 text-muted-foreground">
						Type <code className="text-foreground">/</code> in the composer to
						open the command palette, then use arrows and Enter to run a
						command.
					</p>
				</div>
			</aside>
...
function ModelsContent({
	...
	const availableOptions = useMemo<ModelOption[]>(() => {
		...
		if (
			Array.isArray(data?.availableOptions) &&
	...
	const hasConcreteSessionId =
		typeof currentSessionId === "string" && currentSessionId.trim().length > 0;
		...
		try {
			...
			const message =
				error instanceof Error
					? error.message
					: "Unable to change the model right now.";
	...
	return (
		<div className="flex h-full min-h-0 flex-col gap-3">
			...
			<div className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-muted/20 px-3.5 py-2.5">
				...
				>
					<RefreshCw
						className={`h-4 w-4 ${providerModelsRefreshing ? "animate-spin" : ""}`}
					/>
				</Button>
			</div>
							...
							return (
								...
								>
									...
									{option.label && option.label !== option.value && (
										<span className="mt-1 text-xs font-medium text-foreground/85">
											{option.label}
										</span>
									)}
									{option.description && (
...
function StatusContent({ data }: { data: StatusCommandData }) {
	...
	const rows = [
		{
			label: "Package",
			value: data.packageName || "claude-code-ui",
			icon: Package,
		},
		{
			label: "Version",
		...
		{
			label: "Provider",
			value: getProviderLabel(data.provider, data.provider || "Unknown"),
			icon: Server,
		...
		{
			label: "Node.js",
			value: data.nodeVersion || "Unknown",
			icon: TerminalSquare,
		},
		{ label: "Platform", value: data.platform || "Unknown", icon: Activity },
		{
			label: "Memory",
			value:
				typeof memoryRssMb === "number" ? `${memoryRssMb} MB RSS` : "Unknown",
			icon: Gauge,
		},
	];
...
export default function CommandResultModal({
	...
	const modalMeta = {
		help: {
			...
			subtitle:
				"Search built-ins, syntax patterns, and command usage without leaving the chat.",
			icon: CircleHelp,
		},
		models: {
		...
		status: {
			...
			title: "System Status",
			subtitle:
				"Version, provider, runtime, and environment details in one place.",
			icon: Activity,
		},
	} as const;
		...
		<Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="flex h-[min(92dvh,48rem)] w-[calc(100vw-1rem)] max-w-5xl flex-col overflow-hidden rounded-3xl border-border/80 bg-popover/95 p-0 shadow-2xl backdrop-blur-xl sm:w-[min(94vw,64rem)]">
				...
				<div className="settings-content-enter min-h-0 flex-1 overflow-hidden px-4 py-4 sm:px-6 sm:py-5">
					{payload?.kind === "help" && (
						<HelpContent data={payload.data as HelpCommandData} />
					)}
					{payload?.kind === "models" && (
						<ModelsContent
					...
					{payload?.kind === "status" && (
						<StatusContent data={payload.data as StatusCommandData} />
					)}
				</div>
