...
const isManagedServer = (server: ProviderMcpServer): boolean =>
	server.name.startsWith("cloudcli-");
...
function ConfigLine({ label, children }: { label: string; children: string }) {
	...
	return (
		<div>
			{label}: <code className="rounded bg-muted px-1 text-xs">{children}</code>
		</div>
	);
}
							...
							<div className="flex items-start justify-between">
								<div className="min-w-0 flex-1">
									...
									<div className="space-y-1 text-sm text-muted-foreground">
										{!managed && (
											<>
												<ConfigLine label={t("mcpServers.config.command")}>
													{server.command || ""}
												</ConfigLine>
												<ConfigLine label={t("mcpServers.config.url")}>
													{server.url || ""}
												</ConfigLine>
												<ConfigLine label={t("mcpServers.config.args")}>
													{(server.args || []).join(" ")}
												</ConfigLine>
												<ConfigLine label="Cwd">{server.cwd || ""}</ConfigLine>
												{server.env && Object.keys(server.env).length > 0 && (
													<ConfigLine
														label={t("mcpServers.config.environment")}
													>
														{Object.entries(server.env)
															.map(
																([key, value]) => `${key}=${maskSecret(value)}`,
															)
															.join(", ")}
													</ConfigLine>
												)}
												{server.envVars && server.envVars.length > 0 && (
													<ConfigLine label="Env Vars">
														{server.envVars.join(", ")}
													</ConfigLine>
												)}
											</>
