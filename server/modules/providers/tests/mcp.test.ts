 ...
 */
test("providerMcpService handles codex MCP TOML config and capability validation", {
	concurrency: false,
}, async () => {
	...
	try {
		...
		assert.equal(
			(userServers["codex-user-stdio"] as Record<string, unknown>).command,
			"python",
		);
 ...
 */
test("providerMcpService global adder targets only Codex and rejects unsupported transports", {
	concurrency: false,
}, async () => {
	...
	try {
		...
		assert.ok(
			(codexProject.mcp_servers as Record<string, unknown>)["global-http"],
		);
