...
test('providerSkillsService lists codex repository, user, and system skills', { concurrency: false }, async () => {
  ...
  try {
    await writeSkill(path.join(workspacePath, '.agents', 'skills'), 'cwd-dir', 'codex-cwd', 'Cwd skill');
    await writeSkill(path.join(repoRoot, 'packages', '.agents', 'skills'), 'parent-dir', 'codex-parent', 'Parent skill');
    await writeSkill(path.join(repoRoot, '.agents', 'skills'), 'root-dir', 'codex-root', 'Root skill');
    await writeSkill(path.join(tempRoot, '.agents', 'skills'), 'user-dir', 'codex-user', 'User skill');
    await writeSkill(path.join(tempRoot, '.codex', 'skills'), 'home-dir', 'codex-home-user', 'Home skill');
    await writeSkill(path.join(tempRoot, '.codex', 'skills', '.system'), 'system-dir', 'codex-system', 'System skill');
 ...
 */
test('providerSkillsService manages Codex global skills safely', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(
      createdSkill.sourcePath.endsWith(path.join('.agents', 'skills', 'uploaded-codex-folder', 'SKILL.md')),
      true,
    );
    assert.equal(
    ...
    const fallbackSkills = await providerSkillsService.addProviderSkills('codex', {
      entries: [{
    ...
    const pendingBatchPath = path.join(tempRoot, '.agents', 'skills', 'pending-batch', 'SKILL.md');
    await assert.rejects(
    ...
    assert.equal(listed.some((skill) => skill.name === 'replacement'), true);
    ...
    const removedMissing = await providerSkillsService.removeProviderSkill('codex', {
      directoryName: 'uploaded-codex-folder',
    });
