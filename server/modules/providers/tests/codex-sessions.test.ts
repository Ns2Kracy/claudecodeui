...
async function withIsolatedDatabase(
  ...
  const tempDirectory = await mkdtemp(
    path.join(os.tmpdir(), "codex-provider-db-"),
  );
...
const writeCodexTranscript = async (
  ...
  const lines: string[] = [
    JSON.stringify({
      type: "session_meta",
      payload: { id: codexSessionId, cwd: workspacePath },
    }),
  ];
...
test("Codex synchronizer titles app-created sessions from the first user message", {
  concurrency: false,
}, async () => {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "codex-session-sync-app-"),
  );
  ...
  try {
    ...
    await withIsolatedDatabase(async () => {
      ...
      assert.equal(
        sessionsDb.getSessionById("app-1")?.custom_name,
        "Fix the login redirect bug",
      );
    });
  } finally {
...
test("Codex synchronizer skips sub-agent rollout files", {
  concurrency: false,
}, async () => {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "codex-session-sync-subagent-"),
  );
  const workspacePath = path.join(tempRoot, "workspace");
  await mkdir(workspacePath, { recursive: true });
  const restoreHomeDir = patchHomeDir(tempRoot);
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        payload: {
          ...
          source: {
            subagent: {
              thread_spawn: { parent_thread_id: "codex-parent-1", depth: 1 },
            },
          },
        },
      })}\n`,
...
test("Codex synchronizer leaves indexed sessions untitled when no name is available", {
  concurrency: false,
}, async () => {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "codex-session-sync-indexed-"),
  );
  const workspacePath = path.join(tempRoot, "workspace");
  await mkdir(workspacePath, { recursive: true });
  const restoreHomeDir = patchHomeDir(tempRoot);
  ...
  try {
    ...
    await withIsolatedDatabase(async () => {
      ...
      assert.equal(
        sessionsDb.getSessionById("codex-indexed-1")?.custom_name,
...
test("Codex history hides commentary while preserving reasoning and final answers", {
  ...
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "codex-final-answer-history-"),
  );
  const workspacePath = path.join(tempRoot, "workspace");
  await mkdir(workspacePath, { recursive: true });
  const restoreHomeDir = patchHomeDir(tempRoot);
  ...
  try {
    const providerSessionId = "codex-final-answer-1";
    ...
    await writeFile(
      ...
      [
        JSON.stringify({
          type: "session_meta",
          payload: { id: providerSessionId, cwd: workspacePath },
        }),
        JSON.stringify({
          type: "response_item",
          payload: {
        ...
        JSON.stringify({
          ...
          payload: {
            ...
            phase: "commentary",
            content: [
              { type: "output_text", text: "I will inspect the files first." },
            ],
          },
        }),
        JSON.stringify({
          ...
          payload: {
            ...
            phase: "final_answer",
            content: [
              { type: "output_text", text: "Fixed the duplicate output." },
            ],
          },
        }),
        JSON.stringify({
    ...
    await withIsolatedDatabase(async () => {
      sessionsDb.createAppSession("app-final-answer-1", "codex", workspacePath);
      ...
      const history = await new CodexSessionsProvider().fetchHistory(
        "app-final-answer-1",
      );
      ...
      assert.equal(
        visibleContent.includes("I will inspect the files first."),
        false,
      );
    });
  } finally {
...
test("Codex history renders Promise.all shell wrappers as Bash activity", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const execInput =
      'const cmds = ["echo one", "echo two"]; await Promise.all(cmds.map(command => tools.shell_command({ command })));';
    ...
    await withIsolatedDatabase(async () => {
      ...
      const toolUses = history.messages.filter(
      ...
      assert.equal(
        toolUses[0].toolInput,
        JSON.stringify({ command: "echo one\necho two" }),
      );
      assert.equal(
        toolResults.some((message) => message.toolCallId === "plan-1"),
        false,
      );
    });
  } finally {
