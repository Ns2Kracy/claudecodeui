...
const EMPTY_CODEX_MODELS: ProviderModelsDefinition = {
	OPTIONS: [],
	DEFAULT: "",
};
	...
	return {
		...
		effort:
			effortValues.length > 0
				? {
						default:
							readOptionalString(model.default_reasoning_level) ?? undefined,
						values: effortValues,
					}
				: undefined,
	};
...
const buildCodexModelsDefinition = (
	...
	const sortedModels = [...models]
		...
		.sort(
			(left, right) =>
				readCodexPriority(left.priority) - readCodexPriority(right.priority),
		);
...
/** Used by the Codex provider registry to read local model metadata without inventing models. */
