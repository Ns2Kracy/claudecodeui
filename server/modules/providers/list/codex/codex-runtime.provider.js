...
function extractCodexTokenBudget(event) {
	...
	const usage =
		info?.total_token_usage || event?.usage?.total_token_usage || event?.usage;
	...
	return {
		...
		total:
			readUsageNumber(
				info?.model_context_window || event?.usage?.model_context_window,
			) || 200000,
		inputTokens,
		outputTokens,
		breakdown: {
...
export async function queryCodex(command, options = {}, ws, context) {
	...
	try {
		...
		const selectedModel =
			catalog.OPTIONS.find((option) => option.value === effectiveModel) || null;
		const allowedEfforts =
			selectedModel?.effort?.values?.map((value) => value.value) || [];
		...
		const turnInput =
			normalizeImageDescriptors(images).length > 0
		...
		const runSession = sessionKey()
			? activeCodexSessions.get(sessionKey())
			: null;
		const runAborted =
			runSession?.status === "aborted" || abortController.signal.aborted;
		if (!runAborted) {
			sendMessage(
				ws,
				createCompleteMessage({
