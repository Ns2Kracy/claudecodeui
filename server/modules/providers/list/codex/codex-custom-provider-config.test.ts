...
test('preserves Codex defaults, other providers, and unmanaged Custom fields while updating managed fields', async () => {
  await withTempConfig(async (configPath) => {
    ...
    await applyCustomCodexProvider({ configPath, baseUrl: 'https://router.example/api/v1', apiKey: 'new-key' });
    await applyCustomCodexProvider({ configPath, baseUrl: 'https://router-2.example/api/v1', apiKey: 'newer-key' });
...
test('rejects invalid TOML without replacing the original config', async () => {
  await withTempConfig(async (configPath) => {
    ...
    await assert.rejects(
      applyCustomCodexProvider({ configPath, baseUrl: 'https://router.example/api/v1', apiKey: 'secret-key' }),
