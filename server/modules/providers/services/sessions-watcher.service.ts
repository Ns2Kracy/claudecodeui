...
async function flushPendingWatcherUpdate(): Promise<void> {
	...
	try {
		...
		console.error(
			"Session watcher refresh failed while broadcasting session_upserted",
			{ error: message },
		);
	} finally {
...
async function onUpdate(
	...
	try {
		...
		console.log(
			`Session synchronization triggered by ${eventType} event for provider "${provider}"`,
			{
				filePath,
		...
		try {
			...
			watcher
				...
				.on("error", (error: unknown) => {
					...
					console.error(`Session watcher error for provider "${provider}"`, {
						error: message,
					});
				});
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(
				`Failed to initialize session watcher for provider "${provider}"`,
				{
					rootPath,
					error: message,
				},
