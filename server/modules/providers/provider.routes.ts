...
const parseMcpUpsertPayload = (
	...
	return {
		...
		envHttpHeaders:
			typeof body.envHttpHeaders === "object" && body.envHttpHeaders !== null
				? Object.fromEntries(
						Object.entries(
							body.envHttpHeaders as Record<string, unknown>,
						).filter(
							(entry): entry is [string, string] =>
								typeof entry[1] === "string",
						),
...
const parseProviderSkillCreatePayload = (
	payload: unknown,
): ProviderSkillCreateInput => {
	if (!payload || typeof payload !== "object") {
		throw new AppError("Request body must be an object.", {
	...
	const entries = rawEntries.map((entry, index) => {
		...
		if (!content.trim()) {
			throw new AppError(
				`Skill entry ${index + 1} must include markdown content.`,
				{
		...
		const files: ProviderSkillCreateFile[] | undefined = rawFiles?.map(
			(file, fileIndex) => {
				if (!file || typeof file !== "object") {
					throw new AppError(
						`Skill entry ${index + 1} file ${fileIndex + 1} must be an object.`,
						{
							code: "INVALID_REQUEST_BODY",
							statusCode: 400,
						},
				...
				const fileContent =
					typeof fileRecord.content === "string" ? fileRecord.content : null;
				const encoding =
					fileRecord.encoding === "utf8" || fileRecord.encoding === "base64"
						? fileRecord.encoding
						: null;
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		const provider = parseProvider(req.params.provider);
		const bypassCache =
			parseOptionalBooleanQuery(req.query.bypassCache, "bypassCache") ?? false;
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const stored = await providerModelsService.setSessionModel(
			provider,
			sessionId,
			model,
		);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const skills = await providerSkillsService.addProviderSkills(
			provider,
			input,
		);
		res.json(createApiSuccessResponse({ provider, skills }));
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const groupedServers = await providerMcpService.listProviderMcpServers(
			provider,
			{ workspacePath },
		);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const server = await providerMcpService.upsertProviderMcpServer(
			provider,
			payload,
		);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		if (payload.scope === "local") {
			throw new AppError(
				'Global MCP add supports only "user" or "project" scopes.',
				{
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		const sessionId = parseSessionId(req.params.sessionId);
		const result =
			await providerTokenUsageService.getSessionTokenUsage(sessionId);
		res.json(createApiSuccessResponse(result));
	}),
);
...
router.delete(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const deletedFromDisk =
			parseOptionalBooleanQuery(req.query.deletedFromDisk, "deletedFromDisk") ??
			force;
