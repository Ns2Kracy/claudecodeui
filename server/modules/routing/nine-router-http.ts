...
const OPERATIONS: Record<NineRouterOperation, OperationDefinition> = {
	...
	providerModels: {
		method: "GET",
		path: (input) => `/api/providers/${encodedId(input)}/models`,
	},
	oauthAuthorize: { method: "GET", path: oauthAuthorizePath },
	oauthExchange: {
		method: "POST",
		path: (input) => `/api/oauth/${encodedProvider(input)}/exchange`,
	},
	oauthDeviceCode: {
		method: "GET",
		path: (input) => `/api/oauth/${encodedProvider(input)}/device-code`,
	},
	oauthPoll: {
		method: "POST",
		path: (input) => `/api/oauth/${encodedProvider(input)}/poll`,
	},
	providerNodesList: { method: "GET", path: fixedPath("/api/provider-nodes") },
	providerNodeCreate: {
		method: "POST",
		path: fixedPath("/api/provider-nodes"),
	},
	providerNodeValidate: {
		method: "POST",
		path: fixedPath("/api/provider-nodes/validate"),
	},
	providerNodeUpdate: {
		method: "PUT",
		path: (input) => `/api/provider-nodes/${encodedId(input)}`,
	},
	providerNodeDelete: {
		method: "DELETE",
		path: (input) => `/api/provider-nodes/${encodedId(input)}`,
...
function timedOut(
	...
	return new AppError(
		errorMessage(operation, origin, `timed out during ${stage}`),
		{
...
function redirectRejected(
	...
	return new AppError(
		errorMessage(operation, origin, "returned a forbidden redirect"),
		{
...
function responseTooLarge(
	...
	return new AppError(
		errorMessage(operation, origin, "returned an oversized response"),
		{
...
function invalidResponse(
	...
	return new AppError(
		errorMessage(operation, origin, "returned an invalid JSON response"),
		{
...
function mergedTimeouts(
	overrides: Partial<RequestTimeouts> | undefined,
): RequestTimeouts {
...
export async function requestNineRouterJson(
	...
	const target = await validateRoutingTarget(
		input.baseUrl,
	...
	const pinnedLookup: LookupFunction = (_hostname, options, callback) => {
		if (options.all) {
			callback(null, [
				{ address: target.pinnedAddress, family: target.family },
			]);
		} else {
	...
	return new Promise<NineRouterHttpResult>((resolve, reject) => {
		...
		try {
				...
				(response) => {
					...
					response.on("data", (chunk: Buffer | string) => {
						...
						if (receivedBytes > MAX_BODY_BYTES) {
							response.destroy(
								responseTooLarge(input.operation, target.origin),
							);
							fail(responseTooLarge(input.operation, target.origin));
							return;
						}
					...
					response.once("aborted", () =>
						fail(unreachable(input.operation, target.origin)),
					);
					response.once("error", (error) => {
						fail(
							error instanceof AppError
								? error
								: unreachable(input.operation, target.origin),
						);
					});
					response.once("end", () => {
						clearTimeout(bodyTimer);
			...
			reject(
				error instanceof AppError
					? error
					: operationFailed(input.operation, target.origin),
			);
			return;
		}
