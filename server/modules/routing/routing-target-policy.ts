...
function targetBlocked(): AppError {
	return new AppError("The configured Router target is not permitted", {
...
function targetUnreachable(): AppError {
	return new AppError("The configured Router target could not be resolved", {
...
export async function validateRoutingTarget(
	...
	const allowedHttpHosts = new Set(
		(options.allowedHttpHosts ?? []).map(normalizeAllowedHost),
	);
	...
	const allLoopback = answers.every((answer) =>
		matchesBlockList(LOOPBACK_ADDRESSES, answer),
	);
	...
	for (const answer of answers) {
		...
		if (
			!permittedLoopback &&
			!hostException &&
			!matchesCidr(answer, allowedCidrs)
		) {
			throw targetBlocked();
		}
	}
