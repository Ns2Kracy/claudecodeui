...
test('accepts only http and https origins without credentials, query, or fragment', () => {
  for (const baseUrl of ['http://9router:20128', 'https://router.example.com/base']) {
    assert.equal(createNineRouterSidecarService({ baseUrl, health: async () => ({ ok: true, version: '0.5.45' }) }).getStatus().origin, baseUrl);
  }
  ...
  for (const baseUrl of [
    ...
    assert.throws(
      () => createNineRouterSidecarService({ baseUrl, health: async () => ({ ok: true, version: '0.5.45' }) }),
