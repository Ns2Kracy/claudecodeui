...
test("production compose pins the reproducible 1.37.11 release and official Router", () => {
	const compose = readProjectFile("compose.prod.yaml");

	assert.match(compose, /image:\s+ns2kracy\/cloudcli:1\.37\.11/);
	...
	assert.equal(JSON.parse(readProjectFile("package.json")).version, "1.37.11");
	assert.equal(
