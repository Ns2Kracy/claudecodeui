...
test('connection mutation routes are removed', async () => {
  await withRoutingServer({}, async (baseUrl) => {
    for (const [method, path] of [['PUT', '/connection'], ['POST', '/connection/validations'], ['DELETE', '/connection'], ['POST', '/runtime/restart'], ['PUT', '/bindings/providers/claude'], ['PUT', '/usage-alerts/daily']] as const) {
