...
export function createAgentRouter(
			...
			try {
				...
				const gitEnvironment = githubToken
					? {
							...
							GIT_CONFIG_VALUE_1:
								'!f() { echo username=x-access-token; echo "password=$CLOUDCLI_GITHUB_TOKEN"; }; f',
	...
	async function cleanupProject(projectPath) {
		try {
			const externalProjectsRoot = await fs.realpath(
				path.join(os.homedir(), ".cloudcli", "external-projects"),
			);
	...
	class ResponseCollector {
		...
		/** Returns normalized Codex assistant text messages. */
		...
		getTotalTokens() {
			...
			for (const msg of this.messages) {
				...
				if (tokenBudget) {
					totalInput = Math.max(totalInput, tokenBudget.inputTokens || 0);
					totalOutput = Math.max(totalOutput, tokenBudget.outputTokens || 0);
					totalCacheRead = Math.max(
						totalCacheRead,
						tokenBudget.cacheReadTokens || 0,
					);
					totalCacheCreation = Math.max(
						totalCacheCreation,
						tokenBudget.cacheCreationTokens || 0,
					);
				}
			}
	 ...
	 *                               - If omitted with githubUrl: Auto-generates temporary path in ~/.cloudcli/external-projects/
	 *
	 ...
	 *
	 * @param {string} provider - (Optional) Must be 'codex'. Defaults to 'codex'.
	 *
	 ...
	 *
	 * @param {string} model - (Optional) Model identifier from the configured 9Router catalog.
	 *
	 * @param {string} effort - (Optional) Codex reasoning effort: 'low', 'medium', 'high', or 'xhigh'.
	 ...
	 *   Action: Clones to auto-generated temporary path: ~/.cloudcli/external-projects/<hash>/
	 ...
	 *     - Normalized Codex events such as { kind: "text", content: "..." }
	...
	router.post("/", validateExternalApiKey, async (req, res) => {
		...
		const effort =
			typeof req.body.effort === "string" && req.body.effort.trim()
		...
		try {
			...
			const registrationResult = projectsDb.createProjectPath(
				finalProjectPath,
				null,
			);
			if (registrationResult.outcome === "active_conflict") {
				console.log(
					"Project registration already exists for:",
					finalProjectPath,
				);
			} else {
			...
			if (cleanup && githubUrl && clonedProjectCreated) {
				...
				setTimeout(() => {
					cleanupProject(finalProjectPath);
				}, 5000);
			}
		} catch (error) {
