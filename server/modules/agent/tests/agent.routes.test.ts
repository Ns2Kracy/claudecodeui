...
test("Agent route rejects missing project input before invoking Codex", async () => {
...
test("Agent route rejects removed providers", async () => {
	await withAgentServer(createDependencies(), async (baseUrl) => {
		const response = await fetch(`${baseUrl}/api/agent`, {
			method: "POST",
			headers: { "content-type": "application/json" },
	...
	await withAgentServer(
		createDependencies({
			...
			spawnProcess: ((
				...
				process.nextTick(() =>
					child.emit("error", new Error("expected test failure")),
				);
				return child;
			}) as unknown as AgentDependencies["spawnProcess"],
		}),
...
test("Agent route defaults to Codex and reuses a matching checkout", async () => {
	...
	await withAgentServer(
		createDependencies({
			...
			queryCodex: (async (_message, options) => {
				invokedModel = options.model as string | undefined;
		...
		async (baseUrl) => {
			const response = await fetch(`${baseUrl}/api/agent`, {
				...
				body: JSON.stringify({
					githubUrl: "https://github.com/owner/repo.git",
					projectPath: "/home/test/.cloudcli/external-projects/existing",
					message: "Run",
					stream: false,
					cleanup: true,
				}),
	...
	assert.equal(invokedModel, "default-model");
