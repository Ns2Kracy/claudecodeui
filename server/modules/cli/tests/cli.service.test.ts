...
test("status and help describe the Codex-only runtime", async () => {
	const statusHarness = createHarness();
	assert.equal(await statusHarness.service.run(["status"]), 0);
	const statusOutput = statusHarness.logMessages.join("\n");
	assert.match(statusOutput, /Codex Sessions Folder/);
	assert.doesNotMatch(statusOutput, /CLAUDE_CLI_PATH|Claude Projects Folder/);

	const helpHarness = createHarness();
	assert.equal(await helpHarness.service.run(["help"]), 0);
	const helpOutput = helpHarness.logMessages.join("\n");
	assert.doesNotMatch(helpOutput, /CLAUDE_CLI_PATH|claude-code-ui/);
});
