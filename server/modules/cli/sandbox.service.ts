...
const CODEX_SANDBOX_TEMPLATE = "docker.io/cloudcliai/sandbox:codex";
const CODEX_SANDBOX_SECRET = "openai";

function parseSandboxArguments(
	...
	for (
		...
		if (
			argumentIndex === 0 &&
			subcommands.includes(argument as SandboxOptions["subcommand"])
		) {
	...
	const expandedWorkspace =
		parsedOptions.workspace?.replace(/^~/, homeDirectory) ?? null;
	const name =
		parsedOptions.name ??
		(expandedWorkspace ? path.basename(path.resolve(expandedWorkspace)) : null);
	parsedOptions.agent = "codex";
	const template = parsedOptions.template ?? CODEX_SANDBOX_TEMPLATE;
...
Options:
  -a, --agent <agent>       Deprecated; Codex is always used
...
Advanced usage:
  ...
  or passing prompts to the agent, use sbx directly with the template:
    ...
    sbx run --template docker.io/cloudcliai/sandbox:codex codex ~/my-project --branch my-feature
    sbx run --template docker.io/cloudcliai/sandbox:codex codex ~/project ~/libs:ro --memory 8g

  Full Docker Sandboxes docs: https://docs.docker.com/ai/sandboxes/usage/
`);
}

function requireSandboxName(
...
function publishSandboxPort(
	...
	dependencies.output.log(
		`${terminalTextStyles.info("▶")} Forwarding port ${options.port} → 3001...`,
	);
	try {
...
export function createSandboxCommandService(
	...
	return {
		async execute(argumentsList) {
			const options = parseSandboxArguments(
				argumentsList,
			...
			try {
				...
				dependencies.output.log(
					`   Install it from: ${terminalTextStyles.info("https://docs.docker.com/ai/sandboxes/get-started/")}`,
				);
				dependencies.output.log(
					`   Then run: ${terminalTextStyles.bright("sbx login")}`,
				);
				dependencies.output.log(
					`   And store your API key: ${terminalTextStyles.bright("sbx secret set -g openai")}\n`,
				);
				return 1;
			}
			...
			switch (options.subcommand) {
				...
				case "rm": {
					...
					dependencies.runSandboxCommand(
						[options.subcommand, sandboxName],
						true,
					);
					return 0;
				}
				...
				case "start": {
					...
					dependencies.output.log(
						`${terminalTextStyles.info("▶")} Launching CloudCLI web server...`,
					);
					dependencies.runSandboxCommand([
						"exec",
						sandboxName,
					...
					dependencies.output.log(
						`  ${terminalTextStyles.info("→")} ${terminalTextStyles.bright(`http://localhost:${options.port}`)}\n`,
					);
					return 0;
				}
				...
				case "create": {
					if (!options.workspace) {
						...
						dependencies.output.log(
							`   Example: ${terminalTextStyles.bright("cloudcli sandbox ~/my-project")}\n`,
						);
						return 1;
					}
					...
					const sandboxName = options.name as string;
					const secret = CODEX_SANDBOX_SECRET;
					try {
						...
						if (!secretList.includes(secret)) {
							...
							dependencies.output.log(
								`   Run: ${terminalTextStyles.bright(`sbx secret set -g ${secret}`)}\n`,
							);
							return 1;
						}
					} catch {
					...
					dependencies.output.log(
						`  Agent:     ${terminalTextStyles.info(options.agent)} ${terminalTextStyles.dim(`(${secret} credentials)`)}`,
					);
					dependencies.output.log(
						`  Workspace: ${terminalTextStyles.dim(workspace)}`,
					);
					dependencies.output.log(
						`  Name:      ${terminalTextStyles.dim(sandboxName)}`,
					);
					dependencies.output.log(
						`  Template:  ${terminalTextStyles.dim(options.template)}`,
					);
					dependencies.output.log(
						`  Port:      ${terminalTextStyles.dim(String(options.port))}`,
					);
					if (options.environmentVariables.length > 0) {
						dependencies.output.log(
							`  Env:       ${terminalTextStyles.dim(options.environmentVariables.join(", "))}`,
						);
					}
					dependencies.output.log(terminalTextStyles.dim("─".repeat(50)));
					...
					if (options.environmentVariables.length > 0) {
						dependencies.output.log(
							`${terminalTextStyles.info("▶")} Setting environment variables...`,
						);
						const validEnvironmentVariables =
							options.environmentVariables.filter((environmentVariable) =>
								/^\w+=.+$/.test(environmentVariable),
							);
						...
						const invalidEnvironmentVariables =
							options.environmentVariables.filter(
								(environmentVariable) => !/^\w+=.+$/.test(environmentVariable),
							);
					...
					dependencies.output.log(
						`  ${terminalTextStyles.info("→")} Open ${terminalTextStyles.bright(`http://localhost:${options.port}`)}`,
					);
					dependencies.output.log(
					...
					dependencies.output.log(
						`  ${terminalTextStyles.dim("$")} sbx stop ${sandboxName}`,
					);
					dependencies.output.log(
						`  ${terminalTextStyles.dim("$")} sbx start ${sandboxName}`,
					);
					dependencies.output.log(
						`  ${terminalTextStyles.dim("$")} sbx rm ${sandboxName}`,
					);
					dependencies.output.log(
