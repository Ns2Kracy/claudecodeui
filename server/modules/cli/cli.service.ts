...
function parseCliArguments(argumentsList: string[]): ParsedCliArguments {
	...
	for (
		...
		if (argument === "--port" || argument === "-p") {
			...
			parsedArguments.options.databasePath = argument.slice(
				"--database-path=".length,
			);
			...
			if (argument === "sandbox") {
				parsedArguments.remainingArguments = argumentsList.slice(
					argumentIndex + 1,
				);
				break;
			}
...
function showStatus(dependencies: CliServiceDependencies): void {
	...
	const databasePath =
		environment.DATABASE_PATH || dependencies.defaultDatabasePath;
	const databaseExists = fileSystem.pathExists(databasePath);
	const codexSessionsPath = path.join(
	...
	output.log(
		`       Status: ${
			databaseExists
				...
				: terminalTextStyles.warn(
						"[WARN] Not created yet (will be created on first run)",
					)
		}`,
	);
	...
	if (databaseExists) {
		...
		output.log(
			`       Size: ${terminalTextStyles.dim(`${(databaseStats.size / 1024).toFixed(2)} KB`)}`,
		);
		output.log(
			`       Modified: ${terminalTextStyles.dim(databaseStats.modifiedAt.toLocaleString())}`,
		);
	}

	output.log(`\n${terminalTextStyles.info("[INFO]")} Configuration:`);
	output.log(
		`       SERVER_PORT: ${terminalTextStyles.bright(environment.SERVER_PORT || environment.PORT || "3001")} ${terminalTextStyles.dim(environment.SERVER_PORT || environment.PORT ? "" : "(default)")}`,
	);
	output.log(
		`       DATABASE_PATH: ${terminalTextStyles.dim(environment.DATABASE_PATH || "(using default location)")}`,
	);
	output.log(`\n${terminalTextStyles.info("[INFO]")} Codex Sessions Folder:`);
	output.log(`       ${terminalTextStyles.dim(codexSessionsPath)}`);
	output.log(
		`       Status: ${
			fileSystem.pathExists(codexSessionsPath)
				? terminalTextStyles.ok("[OK] Exists")
	...
	output.log(
		`       Status: ${
			fileSystem.pathExists(environmentFilePath)
				? terminalTextStyles.ok("[OK] Exists")
	...
	output.log(
		`      ${terminalTextStyles.dim(">")} Use ${terminalTextStyles.bright("cloudcli --port 8080")} to run on a custom port`,
	);
	output.log(
		`      ${terminalTextStyles.dim(">")} Use ${terminalTextStyles.bright("cloudcli --database-path /path/to/db")} for custom database`,
	);
	output.log(
		`      ${terminalTextStyles.dim(">")} Run ${terminalTextStyles.bright("cloudcli help")} for all options`,
	);
	output.log(
		`      ${terminalTextStyles.dim(">")} Access the UI at http://localhost:${environment.SERVER_PORT || environment.PORT || "3001"}\n`,
	);
}
...
export function createCliService(
	dependencies: CliServiceDependencies,
): CliApplication {
	const checkForUpdates = async (silent = false): Promise<boolean> => {
		try {
			...
			if (isNewerVersion(latestVersion, currentVersion)) {
				...
				dependencies.output.log(
					`         Run ${terminalTextStyles.bright("cloudcli update")} to update\n`,
				);
				return true;
			}
			if (!silent) {
				dependencies.output.log(
					`${terminalTextStyles.ok("[OK]")} You are on the latest version (${currentVersion})`,
				);
			}
			return false;
		} catch {
			if (!silent) {
				dependencies.output.log(
					`${terminalTextStyles.warn("[WARN]")} Could not check for updates`,
				);
			}
			return false;
		}
	...
	const updatePackage = async (): Promise<void> => {
		dependencies.output.log(
			`${terminalTextStyles.info("[INFO]")} Checking for updates...`,
		);
		const hasUpdate = await checkForUpdates(true);
		if (!hasUpdate) {
			dependencies.output.log(
				`${terminalTextStyles.ok("[OK]")} Already on the latest version (${dependencies.packageMetadata.version})`,
			);
			return;
		}
		...
		try {
			dependencies.output.log(
				`${terminalTextStyles.info("[INFO]")} Updating ${dependencies.packageMetadata.version}...`,
			);
			dependencies.updateGlobalPackage();
			dependencies.output.log(
				`${terminalTextStyles.ok("[OK]")} Update complete! Restart cloudcli to use the new version.`,
			);
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			dependencies.output.error(
				`${terminalTextStyles.error("[ERROR]")} Update failed: ${message}`,
			);
			dependencies.output.log(
				`${terminalTextStyles.tip("[TIP]")} Try running manually: npm update -g @cloudcli-ai/cloudcli`,
			);
		}
	};
	...
	return {
		async run(argumentsList) {
			...
			if (parsedArguments.options.serverPort) {
				dependencies.environment.SERVER_PORT =
					parsedArguments.options.serverPort;
			} else if (
				!dependencies.environment.SERVER_PORT &&
				dependencies.environment.PORT
			) {
				dependencies.environment.SERVER_PORT = dependencies.environment.PORT;
			}
			if (parsedArguments.options.databasePath) {
				dependencies.environment.DATABASE_PATH =
					parsedArguments.options.databasePath;
			}
			...
			switch (parsedArguments.command) {
				case "start":
					...
					return 0;
				case "sandbox":
					return dependencies.sandboxService.execute(
						parsedArguments.remainingArguments,
					);
				...
				default:
					...
					dependencies.output.log(
						'   Run "cloudcli help" for usage information.\n',
					);
					return 1;
			}
		},
