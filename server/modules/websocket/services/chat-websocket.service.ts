...
export function filterAttachmentsToUploadStore(
	...
	const assetsRoot = path.resolve(
		assetsRootOverride ?? getGlobalImageAssetsDir(),
	);
	...
	return normalizeAttachmentDescriptors(attachments).filter((descriptor) => {
		...
		if (!isDirectChild) {
			console.warn(
				`[Chat] Dropping attachment outside the upload store: ${descriptor.path}`,
			);
		}
...
type ProviderRuntimeGateway = {
	...
	resolveToolApproval(
		requestId: string,
		payload: ProviderPermissionDecision,
	): void;
...
async function handleChatSend(
	...
	// Resolve and persist provenance before dispatch. A session-recorded source
	// survives transient catalog failures; legacy/unrecorded selections must be
	// present in the current catalog or the send fails closed.
	if (typeof clientOptions.model === "string" && clientOptions.model.trim()) {
		const model = clientOptions.model.trim();
		const source = await providerModelsService.resolveModelSource(
			provider,
			model,
			sessionId,
		);
		await providerModelsService.setSessionModel(
			provider,
			sessionId,
			model,
			source,
		);
		clientOptions.modelSource = source;
	}
	...
	const verifiedAttachments =
		filterAttachmentsToUploadStore(attachmentCandidates);
	const uniqueAttachments = verifiedAttachments.filter(
		(descriptor, index, all) =>
			all.findIndex((candidate) => candidate.path === descriptor.path) ===
			index,
	);
	...
	const runtimeOptions: AnyRecord = {
		...
		files: uniqueAttachments.filter(
			(descriptor) => !isImageAttachmentDescriptor(descriptor),
		),
		sessionId,
	...
	try {
		...
		console.error(`[Chat] Provider runtime "${provider}" failed`, {
			sessionId,
			error: message,
		});
	} finally {
...
async function handleChatAbort(
	...
	if (!sessionId) {
		sendProtocolError(
			ws,
			"SESSION_ID_REQUIRED",
			"chat.abort requires a sessionId.",
		);
		return;
	}
	...
	if (!run || run.status !== "running") {
		sendProtocolError(
			ws,
			"NO_ACTIVE_RUN",
			`Session "${sessionId}" has no active run.`,
			sessionId,
		);
		return;
	}
...
function handleChatSubscribe(
	...
	for (const target of targets) {
		...
		const sessionId =
			typeof (target as AnyRecord).sessionId === "string"
		...
		const pendingPermissions =
			dependencies.runtime.getPendingApprovalsForSession(sessionId);
