...
test("session archive queries hide archived rows from active project views", async () => {
	await withIsolatedDatabase(() => {
		...
		const activeProjectSessions = sessionsDb.getSessionsByProjectPath(
			"/workspace/demo-project",
		);
		const allProjectSessions =
			sessionsDb.getSessionsByProjectPathIncludingArchived(
				"/workspace/demo-project",
			);

		assert.deepEqual(
			activeSessions.map((session) => session.session_id),
			["session-active"],
		);
		assert.deepEqual(
		...
		assert.deepEqual(
			activeProjectSessions.map((session) => session.session_id),
			["session-active"],
		);
		assert.deepEqual(
		...
		assert.equal(
			sessionsDb.countSessionsByProjectPath("/workspace/demo-project"),
			1,
		);
	});
});
...
test("session model provenance is stored with the selected model", async () => {
	await withIsolatedDatabase(() => {
		sessionsDb.createAppSession(
		...
		sessionsDb.setSessionModel("session-model-source", "cx/gpt-5", "9router");

		const row = sessionsDb.getSessionById("session-model-source");
		assert.equal(row?.model, "cx/gpt-5");
		assert.equal(row?.model_source, "9router");
	});
});
