...
type SessionRow = {
	...
	model: string | null;
	/** Internal dispatch provenance; NULL only for legacy/unresolved rows. */
...
const SESSION_ROW_COLUMNS =
	"session_id, provider, provider_session_id, project_path, jsonl_path, custom_name, model, model_source, isArchived, created_at, updated_at";
...
function normalizeSessionRow<T extends SessionRow | null | undefined>(
	row: T,
): T {
	if (!row) {
		return row;
...
function normalizeProjectPathForProvider(
	provider: string,
	projectPath: string,
): string {
	void provider;
...
export const sessionsDb = {
	...
	createSession(
		...
		const normalizedProjectPath = normalizeProjectPathForProvider(
			provider,
			projectPath,
		);
	...
	assignProviderSessionId(sessionId: string, providerSessionId: string): void {
		...
		const merge = db.transaction(() => {
           ...
           LIMIT 1`,
				...
				.get(providerSessionId, providerSessionId, sessionId) as
				| SessionRow
				| undefined;
	...
	setSessionModel(
		sessionId: string,
		model: string,
		source: "native" | "9router" | null = null,
	): void {
		const db = getConnection();
		db.prepare(
			`UPDATE sessions
	...
	deleteSessionsByProjectPath(projectPath: string): void {
		const db = getConnection();
		const normalizedProjectPath = normalizeProjectPath(projectPath);
		db.prepare(`DELETE FROM sessions WHERE project_path = ?`).run(
			normalizedProjectPath,
		);
	},
