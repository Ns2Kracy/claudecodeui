...
export function createCommandsRouter(
	dependencies: CommandsRouterDependencies,
): express.Router {
	...
	const builtInCommands = [
		{
			name: "/help",
			description: "Show help documentation for Codex",
			namespace: "builtin",
		...
		{
			name: "/memory",
			description: "Open AGENTS.md instructions for editing",
			namespace: "builtin",
			metadata: { type: "builtin" },
		},
		{
...
- Project: \`.codex/commands/\` (project-specific)
- User: \`~/.codex/commands/\` (available in all projects)
...
`;
		...
		"/models": (args, context) =>
			executeModelsCommand(args, context, providerModelsService),
		...
		"/cost": async (args, context) => {
			...
			const directInputTokens =
				Number(normalizedInputValue ?? tokenUsage.input_tokens ?? 0) || 0;
		...
		"/memory": async (args, context) => {
			...
			if (!projectPath) {
				return {
					...
					data: {
						...
						message: "Please select a project to access its AGENTS.md file",
					},
				};
			}
			...
			return {
				...
				data: {
					...
					exists,
					message: exists
						? `Opening AGENTS.md at ${agentsMdPath}`
						: `AGENTS.md not found at ${agentsMdPath}. Create it to store project-specific instructions.`,
				},
			};
		},
	...
	router.post("/list", async (req, res) => {
		try {
			...
			// Scan project-level Codex commands.
			if (projectPath) {
				const projectCommandsDir = path.join(projectPath, ".codex", "commands");
			...
			// Scan user-level Codex commands.
			const homeDir = os.homedir();
			const userCommandsDir = path.join(homeDir, ".codex", "commands");
			...
			// Security: validate commandPath is within allowed directories
			{
				...
				if (!(isUnder(userBase) || (projectBase && isUnder(projectBase)))) {
					return res.status(403).json({
						error: "Access denied",
						message: "Command must be in .codex/commands directory",
					});
				}
			}
