...
export function createGitRouter(
	dependencies: GitRouterDependencies,
): express.Router {
	const fs = dependencies.fileSystem;
	const spawn = dependencies.spawnProcess;
	const projectsDb = {
		getProjectPathById: dependencies.resolveProjectPathById,
	};
	...
	async function validateGitRepository(projectPath) {
		...
		try {
			...
			const { stdout: insideWorkTreeOutput } = await spawnAsync(
				"git",
	...
	async function repositoryHasCommits(projectPath) {
		try {
			await spawnAsync("git", ["rev-parse", "--verify", "HEAD"], {
				cwd: projectPath,
			});
			return true;
		} catch (error) {
	...
	function parseStatusFilePaths(statusOutput) {
		return statusOutput
			...
			.map((line) => {
				...
				return normalizeRepositoryRelativeFilePath(
					renamedFilePath || statusPath,
				);
			})
			.filter(Boolean);
	}
	...
	function buildFilePathCandidates(projectPath, repositoryRootPath, filePath) {
		...
		const projectRelativePath = normalizeRepositoryRelativeFilePath(
			path.relative(repositoryRootPath, projectPath),
		);
	...
	async function resolveRepositoryFilePath(projectPath, filePath) {
		...
		const candidateFilePaths = buildFilePathCandidates(
			projectPath,
			repositoryRootPath,
			filePath,
		);
		...
		if (!normalizedFilePath.includes("/")) {
			const { stdout: repositoryStatusOutput } = await spawnAsync(
				"git",
				["status", "--porcelain"],
				{ cwd: repositoryRootPath },
			);
			...
			const suffixMatches = changedFilePaths.filter(
				(changedFilePath) =>
					...
					changedFilePath.endsWith(`/${normalizedFilePath}`),
			);
	...
	router.get("/status", async (req, res) => {
		...
		try {
			...
			const { modified, added, deleted, untracked, staged } =
			...
			const isNotGitRepository =
				error instanceof AppError && error.code === "NOT_A_GIT_REPOSITORY";
			// A project without a repository is an expected state, not a failure.
			if (!isNotGitRepository) {
				console.error("Git status error:", error);
			}
			res.json({
				error: isNotGitRepository
					? "Not a git repository"
					: "Git operation failed",
				details: isNotGitRepository
					? error.message
	...
	router.post("/init", async (req, res) => {
		...
		try {
			...
			if (isAlreadyRepository) {
				return res.json({
					success: true,
					output: "Repository already initialized",
				});
			}

			const { stdout, stderr } = await spawnAsync("git", ["init"], {
				cwd: projectPath,
			});
	...
	router.get("/diff", async (req, res) => {
		...
		try {
			...
			const { repositoryRootPath, repositoryRelativeFilePath } =
	...
	router.post("/commit", async (req, res) => {
		...
		try {
			...
			for (const file of files) {
				const { repositoryRelativeFilePath } = await resolveRepositoryFilePath(
					projectPath,
					file,
				);
				await spawnAsync("git", ["add", "--", repositoryRelativeFilePath], {
					cwd: repositoryRootPath,
				});
			}
	...
	router.post("/stage", async (req, res) => {
		...
		if (!project || !Array.isArray(files) || files.length === 0) {
			return res
				.status(400)
				.json({ error: "Project id and files are required" });
		}
		...
		try {
			...
			for (const file of files) {
				...
				if (hasCommits) {
					await spawnAsync(
						"git",
						["reset", "HEAD", "--", repositoryRelativeFilePath],
						{ cwd: repositoryRootPath },
					);
				} else {
	...
	router.post("/revert-local-commit", async (req, res) => {
		...
		try {
			...
			try {
				...
				await spawnAsync("git", ["reset", "--soft", "HEAD~1"], {
					cwd: projectPath,
				});
			} catch (error) {
				...
				await spawnAsync("git", ["update-ref", "-d", "HEAD"], {
					cwd: projectPath,
				});
			}
	...
	router.get("/branches", async (req, res) => {
		...
		try {
			...
			// Get all branches
			const { stdout } = await spawnAsync("git", ["branch", "-a"], {
				cwd: projectPath,
			});
	...
	router.post("/checkout", async (req, res) => {
		...
		if (!project || !branch) {
			return res
				.status(400)
				.json({ error: "Project id and branch are required" });
		}
		...
		try {
			...
			const { stdout } = await spawnAsync("git", ["checkout", branch], {
				cwd: projectPath,
			});
	...
	router.post("/create-branch", async (req, res) => {
		...
		if (!project || !branch) {
			return res
				.status(400)
				.json({ error: "Project id and branch name are required" });
		}
		...
		try {
			...
			validateBranchName(branch);
			const { stdout } = await spawnAsync("git", ["checkout", "-b", branch], {
				cwd: projectPath,
			});
	...
	router.post("/delete-branch", async (req, res) => {
		...
		try {
			...
			const { stdout: currentBranch } = await spawnAsync(
				"git",
				["branch", "--show-current"],
				{ cwd: projectPath },
			);
			if (currentBranch.trim() === branch) {
				return res
					.status(400)
					.json({ error: "Cannot delete the currently checked-out branch" });
			}

			const { stdout } = await spawnAsync("git", ["branch", "-d", branch], {
				cwd: projectPath,
			});
			res.json({ success: true, output: stdout });
		} catch (error) {
	...
	router.get("/commit-diff", async (req, res) => {
		...
		if (!project || !commit) {
			return res
				.status(400)
				.json({ error: "Project id and commit hash are required" });
		}
		...
		try {
			...
			const { stdout } = await spawnAsync("git", ["show", commit], {
				cwd: projectPath,
			});
	...
	/**
	 * Generates a commit message using Codex.
...
Generate the commit message:`;
		...
		try {
			...
			console.log("🚀 Calling Codex to generate a commit message");
	...
	router.get("/remote-status", async (req, res) => {
		...
		try {
			...
			const { stdout: remoteOutput } = await spawnAsync("git", ["remote"], {
				cwd: projectPath,
			});
	...
	router.post("/fetch", async (req, res) => {
		...
		try {
			...
			const { stdout } = await spawnAsync("git", ["fetch", remoteName], {
				cwd: projectPath,
			});
	...
	router.post("/publish", async (req, res) => {
		...
		try {
			...
			try {
				const { stdout } = await spawnAsync("git", ["remote"], {
					cwd: projectPath,
				});
	...
	router.post("/discard", async (req, res) => {
		...
		try {
			...
			if (status === "??") {
				...
				await spawnAsync("git", ["restore", "--", repositoryRelativeFilePath], {
					cwd: repositoryRootPath,
				});
	...
	router.post("/delete-untracked", async (req, res) => {
		...
		try {
			...
			if (stats.isDirectory()) {
				...
				res.json({
					success: true,
					message: `Untracked directory ${repositoryRelativeFilePath} deleted successfully`,
				});
			} else {
				await fs.unlink(filePath);
				res.json({
