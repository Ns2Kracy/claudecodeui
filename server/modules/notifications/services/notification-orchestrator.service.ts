...
type NotificationPreferences = ReturnType<
  typeof notificationPreferencesDb.getPreferences
>;
...
function normalizeNotificationSession(
  event: NotificationEvent,
): NotificationEvent {
...
function resolveSessionName(event: NotificationEvent): string | null {
  ...
  return normalizeSessionName(
    sessionsDb.getSessionName(event.sessionId, event.provider),
  );
}
...
export function buildNotificationPayload(
  ...
  const providerLabel =
    PROVIDER_LABELS[normalizedEvent.provider] || "Assistant";
  const sessionName = resolveSessionName(normalizedEvent);
  const message =
    codeMap[normalizedEvent.code] || "You have a new notification";
...
export function notifyUserIfEnabled({
  ...
  const enabledChannels = notificationChannels.filter((channel) =>
    channel.isEnabled(preferences),
  );
  ...
  for (const channel of enabledChannels) {
    Promise.resolve(
      channel.send({ userId, event: normalizedEvent, payload }),
    ).catch((error: unknown) => {
