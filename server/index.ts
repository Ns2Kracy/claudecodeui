...
const RUNNING_VERSION = (() => {
	try {
		return (
			JSON.parse(fs.readFileSync(path.join(APP_ROOT, "package.json"), "utf8"))
				.version || null
		);
	} catch {
		return null;
...
const gitRoutes = createGitModule({ queryCodex });
const agentRoutes = createAgentModule({ queryCodex });
...
app.use(
	express.static(path.join(APP_ROOT, "dist"), {
...
async function startServer() {
	try {
		...
		if (isProduction) {
			console.log(
				`${terminalTextStyles.info("[INFO]")} To run in production mode, go to http://${DISPLAY_HOST}:${SERVER_PORT}`,
			);
		}

		console.log(
			`${terminalTextStyles.info("[INFO]")} To run in development mode with hot-module replacement, go to http://${DISPLAY_HOST}:${VITE_PORT}`,
		);
		...
		server.listen(SERVER_PORT, HOST, async () => {
			...
			console.log("");
			console.log(
				`${terminalTextStyles.info("[INFO]")} Server URL:  ${terminalTextStyles.bright("http://" + DISPLAY_HOST + ":" + SERVER_PORT)}`,
			);
			console.log(
				`${terminalTextStyles.info("[INFO]")} Installed at: ${terminalTextStyles.dim(appInstallPath)}`,
			);
			console.log(
				`${terminalTextStyles.tip("[TIP]")}  Run "cloudcli status" for full configuration details`,
			);
			console.log("");
		...
		const shutdownRuntimeServices = async () => {
			try {
				...
				console.error(
					"[Browser] Error stopping sessions during shutdown:",
					getErrorMessage(err),
				);
			}
			try {
